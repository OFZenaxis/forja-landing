...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-[70]">
      <div
        ref={barRef}
        className="transition-[background-color,box-shadow,backdrop-filter] duration-300 [&.is-scrolled]:border-b [&.is-scrolled]:border-white/60 [&.is-scrolled]:bg-cream/70 [&.is-scrolled]:shadow-[0_8px_30px_-12px_rgba(124,58,237,0.18)] [&.is-scrolled]:backdrop-blur-xl"
      >
        <nav
        ...
        >
          ...
          <button
            ...
            className="flex h-10 w-10 items-center justify-center rounded-full bg-white/70 text-ink shadow-soft backdrop-blur transition-transform active:scale-95 md:hidden"
          >
            {open ? (
      ...
      {open && (
        <div className="container-px animate-menuIn md:hidden">
          <div className="mt-2 rounded-3xl border border-white bg-cream/95 p-4 shadow-lift backdrop-blur-xl">
              ...
              {LINKS.map((l) => (
                <li key={l.href}>
                  <a
                    ...
                    className="block rounded-2xl px-4 py-3 text-base font-medium text-ink transition-colors hover:bg-brand-50"
                  >
                    {l.label}
                  </a>
                </li>
